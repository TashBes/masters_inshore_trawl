...
dend_list <- get_subdendrograms(x,7)
labels <- lapply(dend_list, labels)
grp<- labels %>%
  plyr::ldply( rbind)%>%
  t()%>%
  ...
  rename(G1 = V1,
         ...
         G6 = V6,
         G7 = V7)%>%
  pivot_longer(cols= 1:7,names_to = "grp", values_drop_na = T )%>%
  column_to_rownames(var = "value")
...
grid.s <- grid %>%
  merge( grp, by = "row.names") %>%
  mutate(grp = if_else(grp == "G3", "G4", grp))
...
ggplot(MDS_xy, aes(MDS1, MDS2 )) +   #, shape =grid.s$grp
  geom_point(size = 6)+
  ggforce::geom_mark_ellipse(aes(MDS1, MDS2, group = grid.s$grp, label = grid.s$grp))+

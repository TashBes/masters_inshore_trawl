...
seasonal_effort_per_ves <- drag %>% 
  ...
  pivot_wider(names_from = grid_code, values_from = n) %>%
  mutate_all(~replace_na(.,0)) %>% 
...
ggplot(MDS_xy, aes(MDS1, MDS2 , shape = seasonal_effort_per_ves$target_code, colour = seasonal_effort_per_ves$docking_date_mm)) +   #, shape =grid.s$grp
  geom_point(size = 6 )+
  ggforce::geom_mark_ellipse(aes(MDS1, MDS2, group = seasonal_effort_per_ves$grp))+
  #  geom_text(label = grid.s$Row.names)+
  theme(legend.title = element_text(size = 14, face = "bold"), 
        legend.text = element_text(size = 12, face = "bold"), 
        legend.position = "right", 
        ...
        panel.border = element_rect(fill = NA, colour = "black"), 
        legend.key=element_blank()) + 
  labs(x = "MDS 1", y = "MDS 2", colour = "Month", shape = "target species") +
  scale_colour_brewer(palette = "Paired", aesthetics = "colour")
